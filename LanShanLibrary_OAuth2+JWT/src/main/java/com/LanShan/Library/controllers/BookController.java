...
public class BookController {
    ...
    public JSONObject statistics(){
        ...
        List<ImmutablePair<Object, Float>> te1;
        JSONObject json = new JSONObject();
        ...
        Map<Object, Long> mostBorrowResult = new HashMap<>();
        te = userMapper.getMostBorrowBook();
        for(Pair<Object, Long> pair : te){
            mostBorrowResult.put(pair.getLeft(), pair.getRight());
        }
        json.put("借阅最多的图书 ",mostBorrowResult);
        Map<Object, Long> mostCommentResult = new HashMap<>();
        te = userMapper.getMostCommentBook();
        for(Pair<Object, Long> pair : te){
            mostCommentResult.put(pair.getLeft(), pair.getRight());
        }
        json.put("评论最多的图书 ",mostCommentResult);
        Map<Object, Float> highScoreBooks = new HashMap<>();
        te1 = userMapper.getHighScoreBook();
        for(Pair<Object, Float> pair : te1){
            highScoreBooks.put(pair.getLeft(), pair.getRight());
        }
        json.put("分数最高的图书 ",highScoreBooks);
        Map<Object, Long> newestBooks = new HashMap<>();
        te = userMapper.getNewestBook();
        for(Pair<Object, Long> pair : te){
            newestBooks.put(pair.getLeft(), pair.getRight());
        }
        json.put("最新的图书 ",newestBooks);
        Map<Object, Long> mostUserBorrow = new HashMap<>();
        te = userMapper.getMostUserBorrowBook();
        for(Pair<Object, Long> pair : te){
            mostUserBorrow.put(pair.getLeft(), pair.getRight());
        }
        json.put("借阅最多的用户 ",mostUserBorrow);
