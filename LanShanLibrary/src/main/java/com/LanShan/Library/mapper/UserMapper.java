...
//各种sql，还是mybatisplus舒服，原生写着太难蹦了
...
public interface UserMapper {
    ...
    @Insert("INSERT INTO library.users (id, username, password, authority,enabled,introduction,star) VALUES (#{id}, #{username}, #{password}, #{authority},#{enabled},'这个人很懒，什么都没写',0)")
    public Integer addUser(String id, String username, String password, String authority, boolean enabled);
    ...
    public Integer updateUser(String username, String password, String id);
    ...
    @Select("select id,user_id,content,uptime from daily where user_id = #{id}")
    public List<Daily> getdaily(String id);

    @Select("select id,user_id,content,uptime from daily where id = #{id}")
    public Daily getOneDailyById(String id);

    @Select("select id,aim_id,user_id,content,created_at from comments where aim_id=#{id}")
    public List<Comment> getComments(String id);

    @Select("select id,aim_id,user_id,content,created_at from child_comments where aim_id=#{id}")
    ...
    @Select("select id,aim_id,user_id,content,created_at from comments where id=#{id}")
    public Comment getOneCommentsById(String id);

    @Select("select id,aim_id,user_id,content,created_at from child_comments where id=#{id}")
    public Comment getOneChild_CommentsById(String id);


    @Select("select id,aim_id,user_id,content,created_at from child_comments_book where aim_id=#{id}")
    public List<Comment> getChild_Comments_Book(String id);

    @Select("select id,aim_id,user_id,content,created_at from child_comments_book where id=#{id}")
    ...
    @Insert("insert IGNORE into library.bookfavor (user_id,book_id) values (#{user_id},#{book_id})")
